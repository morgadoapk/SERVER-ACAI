...
module.exports = class AdmController {
    ...
    static async addCardapio(req, res) {
        ...
        if (!salada) {
            salada = ['Salada', 'pouca salada', 'Salada vinagrete', 'Salada mista']
        }

        if (!arroz) {
            arroz = ['Arroz', 'pouca arroz', 'Arroz branco', 'Arroz temperado', 'Arroz integral']
        }

        if (!macarrao) {
            macarrao = ['Macarrão', 'pouca marcarrão', 'Macarrão alho e óleo', 'Macarrão parafuso']
        }

        if (!feijao) {
            feijao = ['Feijão', 'pouca feijão', 'Feijão d/ caldo', 'Feijão tropeiro']
        }
